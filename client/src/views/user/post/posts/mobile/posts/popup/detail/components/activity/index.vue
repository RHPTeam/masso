<template>
  <div class="activity" :data-theme="currentTheme">
    <div class="items--header d_flex align_items_center pt_2">
      ...
      <div class="title">Chọn cảm xúc</div>
    ...
    <div class="items--body">
      ...
      <div v-if="isShowActivityDefault === false" class="activity--default position_relative">
        ...
        <div class="suggest position_absolute" v-if="isShowSuggestDefault === true">
          <VuePerfectScrollbar class="show" v-click-outside="closeSuggestActivityDefault">
            <div
      ...
      >
        <div
          class="activity--desc text_center"
          @click="isShowActivityDefault = false"
        >{{ typeActivityDefault.text }}</div>
            ...
            <VuePerfectScrollbar class="show">
              ...
              <div v-else v-for="(item, index) in listActivity" :key="`i-${index}`">
                <div class="item--tag d_flex align_items_center" @click="attachActivity(item)">
                  ...
                  <div class="desc ml_2">{{ item.text }}</div>
                </div>
              </div>
            </VuePerfectScrollbar>
          </div>
  ...
  async created() {
    if (this.post.activity) {
      ...
      await this.$store.dispatch(
        "getListActivityFb",
        this.post.activity.typeActivity.id
      );
    }
  },
  methods: {
