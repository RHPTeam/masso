<template>
  <div class="wrapper position_fixed" :data-theme="currentTheme">
    <VuePerfectScrollbar class="scroll-area" ref="scroll">
      <div v-if="post" class="wrapper--content">
        <div class="item mb_4">
        ...
        <div class="item mb_4">
          ...
          <div class="wrap mt_2">
            ...
            >
              ...
              <li
                class="item more d_flex align_items_center justify_content_between"
                @click="isShowMoreOption = true"
              >
          ...
          <div v-if="post.scrape && post.scrape.length > 0" class="item mb_4">
            ...
            <div class="wrap p_2 mt_2">
              <div class="link d_flex align_items_center">
                <label class="link--name py_1">{{ post.scrape }}</label>
        ...
        <div v-if="linkContent && linkContent.length > 0">
          <div class="question item mb_3" @click="isShowChangeScrape = !isShowChangeScrape">
            <span>Chọn tại đây, nếu bạn muốn thay đổi link chia sẻ sử dụng trong bài viết.</span>
          </div>
          <div v-if="isShowChangeScrape === true">
            <div class="item mb_4">
              <div
                class="mt_2"
              >Bạn chỉ có thể sử dụng 1 link chia sẻ trong bài viết, hãy cân nhắc trước khi lựa chọn. Click vào link để thay đổi.</div>
              <div class="wrap p_2 mt_2">
                <div
                  class="link d_flex align_items_center"
                  v-for="(item, index) in linkContent"
                  :key="`l-${index}`"
                >
                  <label
                    class="link--name py_1"
                    for="radio1"
                    @click="chooseLinkContent(item)"
                  >{{ item }}</label>
        ...
        <div class="footer d_flex align_items_center">
          <div class="item--button mr_auro" @click="closePopupCreateNewPost">Hủy</div>
          <div class="ml_auto d_flex align_items_center">
            <div class="item--button pr_4" @click="savePost">Lưu</div>
            <div class="item--button" @click="saveAndPostNow">Lưu & Đăng ngay</div>
          </div>
        </div>
      </div>
    </VuePerfectScrollbar>
