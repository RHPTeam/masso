<template>
  <div class="detail position_relative" :data-theme="currentTheme">
    <div>
      ...
      <div class="action d_flex align_items_center justify_content_between">
        <div class="left position_relative">
          ...
          <input type="text" placeholder="Tìm kiếm" v-model="search" @keydown.enter="searchPosts" />
        </div>
      ...
      <div class="body">
        ...
        <div v-if="postsPageInfinite">
          <div
            class="result--item empty d_flex align_items_center px_3 py_2 justify_content_center"
            v-if="postsPageInfinite.length === 0"
          >Không có dữ liệu</div>

          <VuePerfectScrollbar class="infinite scroll-categories" @ps-y-reach-end="loadMore">
            <div
              class="result--item px_3 py_2 d_flex align_items_center"
              v-for="(post, index) in postsPageInfinite"
              :key="`p-${index}`"
            >
              <div class="content">
                <div class="name--text">{{post.title}}</div>
                <div class="text_left pr_3">
                  <span
              ...
              <div class="action">
                <div
                  class="btn--choose checked text_center m_auto"
                  @click="unselectPost(post._id)"
                  v-if="checkSelectedPost(post._id)"
                ></div>
                <div class="btn--choose text_center m_auto" @click="selectPost(post)" v-else></div>
              </div>
            </div>
    ...
    <div class="footer--back d_flex">
      <div class="top--left ml_auto" @click="back">Tiếp tục</div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    checkSelectedPost(id) {
      return (
        this.event.post_custom.filter(item => {
          return item._id === id;
        }).length > 0
      );
    },
