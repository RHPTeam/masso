<template>
  <div class="modal--wrapper" :data-theme="currentTheme">
    <div class="modal--dialog d_flex justify_content_center align_items_center">
      <div class="modal--content px_4 pb_4 pt_3">
        <!-- Start: Modal Header -->
        <div class="modal--header position_relative">
          <div class="btn--close position_absolute"
               @click="closePopup"
          >
            <icon-base
              height="18px"
              width="18px"
              viewBox="0 0 20 20"
            >
              <icon-cancel></icon-cancel>
            </icon-base>
          </div>
        </div>
        <!-- End: Modal Header -->
        <!-- Start: Modal Body -->
        <div class="modal--body">
          <div class="general--info d_flex mb_3">
            <div class="left">
              <div class="thumbnail mr_3"
              ></div>
            </div>
            <div class="right">
              <div class="right--data">
                <div class="title">{{ product.name }}</div>
                <div class="editor mt_1">bởi {{ product._creator.name }}</div>
                <div class="sale d_flex align_items_center mt_3">
                  <icon-base
                    class="icon--user mr_1"
                    width="16px"
                    height="16px"
                    viewBox="0 0 20 20"
                  >
                    <icon-user></icon-user>
                  </icon-base>
                  <span>{{ product.sale }} đã sử dụng</span>
                </div>
                <div class="price mt_3">{{ product.priceCents }} ₫</div>
              </div>
              <div class="right--btn">
                <div class="btn btn_outline_info">Thêm vào kho</div>
              </div>
            </div>
          </div>
          <div class="functions mt_3">
            <div class="functions--title mb_2">Đặc điểm</div>
            <ul class="list_group pl_3">
              <li class="list_group_item" v-for="(attr, index) in product.attributes" :key="`e-${index}`">
                <span class="font_weight_bold">{{ product.attributes[index].name }}: </span>
                <span>{{ product.attributes[index].value }}</span>
              </li>
            </ul>
          </div>
          <div class="preview mt_3">
            <div class="preview--title mb_2">Xem trước</div>
            <div class="preview--content">{{ product.content }}</div>
          </div>
        </div>
        <!-- End: Modal Body -->
      </div>
    </div>
  </div>
</template>

<script>
export default {
  props: [ "product" ],
  data() {
    return {

    };
  },
  computed: {
    currentTheme() {
      return this.$store.getters.themeName;
    },
    listPost() {
      return this.$store.getters.allProduct;
    }
  },
  methods: {
    closePopup() {
      this.$emit( "closePopup", false );
    }
  },
  async created() {
    await this.$store.dispatch("allProduct");
  }
};
</script>

<style lang="scss" scoped>
@import "./index.style";
.thumbnail{
  background: url("https://hinhanhdepvai.com/wp-content/uploads/2017/05/hot-girl.jpg") no-repeat;
  background-size: cover;
}
</style>

