...
const getters = {
  ...
  verifyPasswordToken: (state) => state.verifyPasswordToken
};
...
const mutations = {
  ...
  setVerifyPasswordToken: (state, payload) => {
    state.verifyPasswordToken = payload;
  }
};
...
const actions = {
  ...
  sendFile: async ({ commit }, payload) => {
    ...
    const result = await AccountServices.show(CookieFunction.getCookie('uid'));
  ...
  verifyPassword: async ({ commit }, payload) => {
    try {
      commit('auth_request');
      const sendPassword = {
        password: payload
