...
const mutations = {
  ...
  set_post_array: (state, payload) => {
    state.post[payload.key] = payload.value;
    state.post[payload.key] = RemoveFunction.removeDuplicateObjectInArray(
      payload.value,
      "uid"
    );
  },
...
const actions = {
  ...
  getPostsPageInfiniteByKey: async ({ commit }, payload) => {
    ...
    const res = await PostServices.searchByKey(
      payload.keyword,
      payload.size,
      payload.page
    );
  ...
  deleteAttachmentPost: async ({ commit }, payload) => {
    await PostServices.deleteAttachmentPost(
      payload.postId,
