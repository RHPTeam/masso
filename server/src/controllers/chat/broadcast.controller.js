...
module.exports = {
  ...
  "index": async ( req, res ) => {
    ...
    const email = secure( res, authorization );
  ...
  "create": async ( req, res ) => {
    ...
    if ( foundBroadcast ) {
      return res.status( 405 ).json( jsonResponse( "Bạn đã tạo broadcast trước đó!", null ) );
    }
  ...
  "update": async ( req, res ) => {
    ...
    if ( !foundBroadcast ) {
      return res.status( 403 ).json( jsonResponse( "Broadcast không tồn tại!", null ) );
    }
    ...
    if ( foundBroadcast.typeBroadCast === "Tin nhắn gửi ngay" ) {
      ...
      if ( !contentItem ) {
        return res.status( 405 ).json( jsonResponse( "Không có nội dung này trong block" ), null );
      }
      ...
      if ( contentItem.typeContent === "subscribe" || contentItem.typeContent === "unsubscribe" ) {
        ...
        if ( checkExist ) {
          return res.status( 405 ).json( jsonResponse( "Một trong số các chuỗi kịch bản không có trong tài khoản của bạn!", null ) );
        }
    ...
    if ( req.query._blockId ) {
      ...
      if ( !block ) {
        return res.status( 403 ).json( jsonResponse( "Block không tồn tại ở Broadcast này!", null ) );
      }
      if ( req.query._status === "true" ) {
        block.status = !block.status;
        await foundBroadcast.save();
        return res.status( 201 ).json( jsonResponse( "Cập nhật broadcast thành công", block ) );
      }
      ...
      if ( req.body.hour || req.body.dateMonth || req.body.hour && req.body.dateMonth ) {
        ...
        if ( ( Date.parse( checkDate ) ) <= Date.now() ) {
          return res.status( 405 ).json( jsonResponse( "Thời gian cập nhật không được nhỏ hơn thời gian hiện tại", null ) );
        }
      }
  ...
  "delete": async ( req, res ) => {
    ...
    if ( req.query._blockId ) {
      ...
      if ( !findBlock ) {
        return res.status( 403 ).json( jsonResponse( "Broadcast của bạn không chứa block này!", null ) );
      }
      ...
      if ( req.query._contentId ) {
        ...
        if ( !findContent ) {
          return res.status( 403 ).json( jsonResponse( "Broadcast của bạn có block không chứa này!", null ) );
        }
